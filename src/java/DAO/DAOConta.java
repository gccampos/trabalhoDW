...
public class DAOConta {
    ...
    public static void updateSaldoAplicacao(Conta conta, double valor) throws SQLException {
    ...
    public static void updateSaldoResgate(Conta conta, double valor) throws SQLException {
        try (PreparedStatement sql = conexao.prepareStatement("update conta set saldo = ? where codigoConta = ?")) {
            sql.setDouble(1, conta.getSaldo() + valor);
            sql.setInt(2, conta.getCodigo());
            sql.executeUpdate();
